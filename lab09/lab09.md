...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   - IP-адрес моего хоста: 192.168.2.108
   - IP-адрес хоста назначения: 128.119.245.12
   - ![alt text](images/image-3.png)
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - ICMP работает на сетевом уровне, а не на транспортном, поэтому он не использует порты (там Type и Code)
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Для ping-запроса (пакет 22) ICMP Type = 8, Code = 0 - Echo Request.
   - поля: Type, Code, Checksum, Identifier, Sequence number, Data.
   - Checksum занимает 2 байта, Identifier занимает 2 байта, Sequence number занимает 2 байта.
   - ![alt text](images/image-3.png)
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Для ping-ответа, например пакет №26, ICMP Type = 0, Code = 0. Это Echo Reply.
   - В этом ICMP-пакете есть поля: Type, Code, Checksum, Identifier, Sequence number и Data.
   - Checksum занимает 2 байта, Identifier занимает 2 байта, Sequence number занимает 2 байта.
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Отличие в том что traceroute отправляет пакеты с разным TTL, а так это тот же ICMP Echo Request, Type = 8, Code = 0
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Type = 11, Code = 0 — Time-to-live exceeded, ещё внутри есть часть исходного пакета- IP-адреса, TTL, протокол ICMP и исходный Echo Request
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Это не ошибки, а ICMP Echo Reply от конечного хоста 128.119.245.12, значит пакет дошёл до адресата
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Скачок задержки между ams2 и bos1 - примерно с 51 ms до 126 ms. Это маршрутизаторы в Амстердаме и Бостоне (логично тк там переход через атлантику), но мы по имени только предполагаем, что это так - это не 100% информация
