...
![alt text](Screens_A/image.png) ![alt text](Screens_A/image-4.png) ![alt text](Screens_A/image-3.png) ![alt text](Screens_A/image-2.png) ![alt text](Screens_A/image-1.png)
...
$L = 56 * 8 = 448$ бит - размер пакета

Скорость формирования голосового потока
$= 128 \text{ Кбит/с} = 128000 \text{ бит/с}$

один пакет собирается хостом A за
$d_{\text{пакет}} = \frac{448}{128000} = 0.0035 \text{ с} = 3.5 \text{ мс}$

Время передачи
$= d_{\text{передачи}} = \frac{448}{10^6} = 0.000448 \text{ с} = 0.448 \text{ мс}$

Задержка $= d_{\text{задержка}} = 5 \text{ мс}$

полная задержка $= d = d_{\text{пакет}} + d_{\text{передачи}} + d_{\text{задержка}} = 3.5 + 0.448 + 5 = 8.948 \text{ мс}$
...
#### Решение
$N = 10 + 1 = 11$ (в буфере в среднем находится $10$ пакетов, и еще $1$ пакет передается)

Средняя задержка ожидания $=d_{\text{ожидания}} = 10 \text{ мс} = 0.01 \text{ с}$

Средняя задержка передачи одного пакета $=d_{\text{передачи}} = \frac{1}{100} = 0.01 \text{ с}$

Общая средняя задержка: $=d = d_{\text{ожидания}} + d_{\text{передачи}} = 0.01 + 0.01 = 0.02 \text{ с}$

$N = a * d \Rightarrow a = \frac{N}{d} = \frac{11}{0.02} = 550$


**Ответ:**

$$
\boxed{a = 550 \text{ пакетов/с}}
$$
...
Время передачи пакета по первой линии $= \frac{L}{R_S}$

Так как пакеты отправляются друг за другом, второй пакет полностью поступит на маршрутизатор на $\frac{L}{R_S}$ позже первого

Так как первая линия - боттлнэк, то вторая линия успевает передать первый пакет до прихода второго полностью, так что временной разрыв между моментами получения клиентом последних битов двух пакетов равен $\frac{L}{R_S}$

### 2.

Теперь $R_C < R_S$

Второй пакет может ждать в буфере маршрутизатора, потому что первая линия доставляет пакеты быстрее, чем вторая линия успевает их передавать дальше

Если первый пакет начинает передаваться сервером в момент $0$, то он полностью приходит к маршрутизатору в момент $\frac{L}{R_S} + d_{\text{распространения}}$, и с этого момента начинает передаваться по второй линии. Передача первого пакета по второй линии закончится в момент $\frac{L}{R_S} + d_{\text{распространения}} + \frac{L}{R_C}$

Если второй пакет отправлен через $T$ секунд после первого, то он полностью поступит на маршрутизатор в момент $T + \frac{L}{R_S} + d_{\text{распространения}}$

Чтобы очередь на вторую линию была нулевой, второй пакет должен приходить не раньше, чем освободится вторая линия. Тогда
$T + \frac{L}{R_S} + d_{\text{распространения}} \ge \frac{L}{R_S} + d_{\text{распространения}} + \frac{L}{R_C} \Rightarrow T \ge \frac{L}{R_C}$

Тогда $T_{\min} = \frac{L}{R_C}$
...
#### Решение
Скорость канала доступа $=R = 15 \text{ Мбит/с}$

Средний размер объекта $=F = 850000 \text{ бит}$

Частота запросов $=B = 16 \text{ запросов/с}$

Средняя задержка в интернете $=d_{\text{интернета}} = 3 \text{ с}$

### 1. 

$\Delta = \frac{F}{R} = \frac{850000}{15000000} = 0.0567 \text{ с}$

### 2.

Средняя задержка доступа $=d_{\text{доступа}} = \frac{\Delta}{1 - \Delta B} = d_{\text{доступа}} = \frac{0.0567}{1 - 0.0567 * 16} = \frac{0.0567}{0.0933}= 0.607 \text{ с}$

Общее среднее время ответа $=d_{\text{общ}} = d_{\text{доступа}} + d_{\text{интернета}}= 0.607 + 3= 3.607 \text{ с}$

### 3.
$m = 0.4$ (я считаю что при попадании в кэш время ответа почти 0)

Новая частота запросов $= B' = m*B = 0.4 * 16 = 6.4 \text{ запросов/с}$

Средняя задержка доступа для промахов кэша $=d'_{\text{доступа}} = \frac{\Delta}{1 - \Delta B'} = \frac{0.0567}{1 - 0.0567 \cdot 6.4} = 0.0889 \text{ с}$

Так как только 40% запросов идет во внешний интернет, тосреднее общее время ответа $=d'_{\text{общ}} = 0.4 * (d'_{\text{доступа}} + 3) = 0.4 * (0.0889 + 3) = 1.236 \text{ с}$
