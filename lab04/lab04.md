...
3. Используя nslookup, найдите веб-сервер, имеющий несколько IP-адресов. Сколько IP-адресов имеет веб-сервер вашего учебного заведения?
   - ![alt text](dns_screens/image-2.png)
   - 1
   - ![alt text](dns_screens/image-3.png)
...
1. Найдите DNS-запрос и ответ на него. С использованием какого транспортного протокола
   они отправлены?
   - UDP
   - ![alt text](dns_screens/image-5.png)
   - ![alt text](dns_screens/image-4.png)
2. Какой порт назначения у запроса DNS?
   - 53
   - ![alt text](dns_screens/image-6.png)
...
6. Посмотрите на последующий TCP-пакет с флагом SYN, отправленный вашим компьютером.
   ...
   - соответствует. Последующий TCP SYN был отправлен на адрес 104.16.44.99 и этот адрес присутствует в DNS-ответе среди ipv4hint (скрин тот же)
7. Веб-страница содержит изображения. Выполняет ли хост новые запросы DNS перед
   загрузкой этих изображений?
   - Да, видны дополнительные DNS-запросы (например к static.ietf.org и analytics.ietf.org)
   - ![alt text](dns_screens/image-11.png)
...
4. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в каждом?
   - 0
   - прямых ответов нет, есть Authority RRs: 1 и Additional RRs: 1
![alt text](dns_screens/image-12.png)
...
2. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся ли в запросе какие-нибудь «ответы»?
   - NS
   - в запросе ответов нет (Answer RRs = 0)
3. Проанализируйте ответное сообщение DNS. Имена каких DNS-серверов университета в
   ...
   - Да, есть только один адрес: 185.44.15.195
![alt text](dns_screens/image-13.png)
![alt text](dns_screens/image-14.png)
...
1. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию? 
   ...
   - 195.70.196.210
   - не совпадает. Локальный DNS-сервер по умолчанию - 192.168.0.1, адрес 195.70.196.210 принадлежит хосту ns2.pu.ru
...
3. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - 2
   - 1) CNAME: www.spbu.ru -> spbu.ru
     2) A: spbu.ru -> 195.70.219.100

![alt text](dns_screens/image-16.png)
![alt text](dns_screens/image-15.png)

### Е. Сервисы whois (2 балла)
1. Что такое база данных whois?
   - это база регистрационных данных об интернет-ресурсах, из которой можно узнать сведения о домене (регистратора, даты регистрации/окончания, статус домена, DNS-серверы)
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - ns.pu.ru
   - ns2.pu.ru
   - whois.nic.ru и Whois ТЦИ  ![alt text](dns_screens/image-17.png)
...
3. Используйте команду nslookup на локальном хосте, чтобы послать запросы трем конкретным
   ...
   - ![alt text](dns_screens/image-18.png)
