...
def is_blocked(target_url: str) -> bool:
    parsed = urlsplit(target_url)
    host = parsed.hostname or ""
    for item in load_blacklist():
        if target_url.startswith(item):
            return True
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def forward(self):
        ...
        if is_blocked(target_url):
            self.send_error_response(403, "Blocked by proxy", target_url)
            return
...
def main() -> None:
    ...
    with HTTPServer(("127.0.0.1", port), ProxyHandler) as server:
        print(f"бл-сервер на http://127.0.0.1:{port}")
