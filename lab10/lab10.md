...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - Длина IP-заголовка = 20 байт, общая длина IP-дейтаграммы = 56 байт, полезная нагрузка IP-дейтаграммы = 36 байт
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Изменяются поля Identification, Time to Live, Header Checksum (TTL меняется группами по 3 пакета)
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются Version, Header Length, Total Length, Source Address, Destination Address и Protocol + поскольку в запуске размер пакета 56 байт и фрагментации нет, не меняются поля Flags и Fragment Offset.
      - Изменяются поля Identification, Time to Live, Header Checksum (как в 2.)
      - Должны оставаться неизменными те поля, которые описывают общий формат и направление отправки пакетов:Version, Header Length, Source Address, Destination Address, Protocol, Total Length
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Identification = 0xa581 = 42369
      - Identification = 0xa582 = 42370
      - Identification = 0xa583 = 42371
      - В моем захвате Identification IP-дейтаграммы увеличивается последовательно на 1: 0xa581, 0xa582, 0xa583
      - Закономерность в данном захвате: для каждой следующей IP-дейтаграммы значение Identification увеличивается на 1
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - Для ICMP-пакета 10 Identification в IP-заголовке = 0xa581 (42369), TTL = 1 (скрин выше)
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Пакет 26: Identification = 0x7ce0 (31968), TTL = 64
   - Пакет 27: Identification = 0x7ce1 (31969), TTL = 64
   - Пакет 28: Identification = 0x7ce2 (31970), TTL = 64
   - Identification не остается неизменным - он увеличивается на 1. TTL остается неизменным - во всех трех пакетах он равен 64
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - 3 фрагмента, это видно по пакетам 55, 56 и 57 - они имеют одинаковые адреса источника и назначения, одинаковый протокол ICMP и одинаковое значение поля Identification = 0x2ad7, но разные Fragment Offset: 0, 1480 и 2960
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Изменяются Total Length, Flags, Fragment Offset и Header Checksum (флаг More Fragments установлен у всех фрагментов, кроме последнего)
